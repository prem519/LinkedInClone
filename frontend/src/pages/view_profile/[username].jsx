...
import { getConnectionRequest, getMyConnectionsRequests } from '@/config/redux/action/authAction';
...
export default function ViewProfilePage({ userProfile }) {
  ...
  const { isCurrentUserInConnection, isConnectionNull } = useMemo(() => {
    const connection = authState.connections.find(
      (user) => user.connectionId._id === userProfile.userId._id
    );

    let inConnection = false;
    let connectionNull = true;

    if (connection) {
      inConnection = true;
      connectionNull = !connection.status_accepted;
    }
    ...
    if (request) {
      ...
      if (request.status_accepted === true) {
        connectionNull = false;
      }
    }

    return { isCurrentUserInConnection: inConnection, isConnectionNull: connectionNull };
        ...
        <DashboardLayout>
          <div className={styles.container}>
            <div className={styles.backDropContainer}>
              <img className={styles.backDrop} src={`${BASE_URL}/${userProfile.userId.profilePicture}`} alt="backdeop" />
            </div>
